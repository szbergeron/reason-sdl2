...
extern "C" {
CAMLprim value caml_createImage(value vPixels) {
  CAMLparam1(vPixels);
...
CAMLprim value caml_destroyImage(value vImage) {
  CAMLparam1(vImage);
...
CAMLprim value caml_getImagePixels(value vImage) {
  CAMLparam1(vImage);
  CAMLlocal1(ret);
  ReglfwImageInfo *image = (ReglfwImageInfo *)vImage;
  int numChannels = 4; // RGBA
  ret = caml_ba_alloc_dims(CAML_BA_UINT8 | CAML_BA_C_LAYOUT, 2, image->data,
                           image->height, image->width * numChannels);
  CAMLreturn(ret);
}

CAMLprim value caml_setImagePixels(value vImage, value vPixels) {
...
CAMLprim value caml_saveImage(value vImage, value vPath) {
  ...
  uint16_t header[3] = {
      (uint16_t)image->width, (uint16_t)image->height,
